...
export const EnhancedChatbotInputSchema = z.object({
    ...
    accessToken: z.string().optional().describe('OAuth access token for the current user. Optional — if absent, email tools are disabled.'),
...
export async function enhancedChatbotResponse(input: EnhancedChatbotInput): Promise<EnhancedChatbotOutput> {
    ...
    const hasEmailAccess = !!accessToken;

    console.log("[Chatbot Flow] Starting generation for query:", query, "| Email access:", hasEmailAccess);

    const modelPrimary = google("gemini-1.5-flash");
    const modelFallback = groq("llama-3.3-70b-versatile");

    const systemPrompt = hasEmailAccess
        ? `You are XeloFlow, an intelligent AI email assistant built into XeloFlow. You have access to the user's real Gmail inbox via tools.
...
- Keep responses concise. Max 3-4 sentences or bullet points unless the user asks for detail.`
        : `You are XeloFlow, an intelligent AI email assistant. The user is not currently signed in, so you cannot access their inbox.

Be helpful, friendly, and professional. You can help with:
- Drafting or improving emails
- Answering questions about email best practices
- General productivity advice

If the user asks about their specific emails, politely let them know they need to sign in first.
Detect the user's language and always respond in the same language.`;

    // Only include email tools when we have a valid access token
    const emailTools = hasEmailAccess ? {
    ...
    // Only add tools + maxSteps when email tools are available
    if (emailTools) {
        generateOptions.tools = emailTools;
        generateOptions.maxSteps = 5;
    }

    const { text } = await generateWithFallback(generateOptions);
